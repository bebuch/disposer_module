...
/// \date 07.05.2009
/// \brief Class template disposer_module::rect
///
...
///
#ifndef _disposer_module_rect_hpp_INCLUDED_
#define _disposer_module_rect_hpp_INCLUDED_
...
namespace disposer_module{
	...
	public:
		...
		rect(disposer_module::size< size_type > const& size):
			size_(size)
			{}
		...
		rect(point< position_type > const& top_left, disposer_module::size< size_type > const& size):
			top_left_(top_left),
			size_(size)
			{}
		...
		/// \brief Get the size
		disposer_module::size< size_type > const size()const{
			return size_;
		}
		...
		void set_size(disposer_module::size< size_type > const& size){
			size_ = size;
		}

		/// \brief Set width and height
		void set_size(size_type const& width, size_type const& height){
			set_size(disposer_module::size< size_type >{width, height});
		}
	...
	private:
		...
		disposer_module::size< size_type > size_;
	};
