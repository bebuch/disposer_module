...
namespace disposer_module{
	...
	public:
		...
		///        and size.height
		constexpr rect(disposer_module::size< size_type > const& size):
			size_(size)
			{}
		...
		constexpr rect(
			point< position_type > const& top_left,
			disposer_module::size< size_type > const& size
		):
			top_left_(top_left),
			size_(size)
			{}
		...
		/// \brief Get the size
		constexpr disposer_module::size< size_type > const size()const{
			return size_;
		}
